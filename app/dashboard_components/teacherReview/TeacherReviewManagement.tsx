...
export default function TeacherReviewManagement({
  ...
  const [openPopover, setOpenPopover] = React.useState<
    null | "teacher" | "student" | "rating" | "date"
  >(null);
    ...
    () =>
      reviews.filter((review) => {
        ...
        const matchesTeacher = filterTeacher
          ? review.teacher_name === filterTeacher
          : true;
        const matchesStudent = filterStudent
          ? review.student_name === filterStudent
          : true;
        const matchesRating = filterRating
          ? Math.round(review.rating) === Number(filterRating)
          : true;
  ...
  const visibleCols =
    1 + (showTeacherCol ? 1 : 0) + (showStudentCol ? 1 : 0) + 1 + 1 + 1;

  const getPopoverPosition = (
    filterName: "teacher" | "student" | "rating" | "date"
  ): PopoverPosition => {
  ...
  if (loading)
    return <div className="text-muted-foreground p-4">Loading reviews...</div>;
  if (error) return <div className="text-destructive p-4">Error: {error}</div>;

  return (
    <div
      className="space-y-6 relative p-4 bg-card rounded-lg shadow-lg"
      ref={rootRef}
    >
      ...
      <div className="overflow-x-auto bg-background rounded-lg border border-border shadow-sm">
        <table className="w-full table-auto">
          <thead className="bg-muted border-b border-border">
            <tr>
              <th className="px-3 py-3 text-left text-xs font-semibold text-foreground uppercase border-r border-border w-16">
                ID
              </th>
              ...
              {showTeacherCol && (
                <th className="px-3 py-3 text-left text-xs font-semibold text-foreground uppercase border-r border-border min-w-[150px]">
                  <div className="flex items-center gap-2 justify-between">
                    TEACHER
                    <button
                      aria-label="label for teacher filter"
                      ref={(el) => {
                        filterButtonRefs.current.teacher = el;
                      }}
                      onClick={() =>
                        setOpenPopover(
                          openPopover === "teacher" ? null : "teacher"
                        )
                      }
                      className={`p-1 rounded hover:bg-background/50 ${
                        filterTeacher ? "text-primary" : "text-muted-foreground"
                      }`}
                    >
              ...
              {showStudentCol && (
                <th className="px-3 py-3 text-left text-xs font-semibold text-foreground uppercase border-r border-border min-w-[150px]">
                  <div className="flex items-center gap-2 justify-between">
                    STUDENT
                    <button
                      aria-label="label for student filter"
                      ref={(el) => {
                        filterButtonRefs.current.student = el;
                      }}
                      onClick={() =>
                        setOpenPopover(
                          openPopover === "student" ? null : "student"
                        )
                      }
                      className={`p-1 rounded hover:bg-background/50 ${
                        filterStudent ? "text-primary" : "text-muted-foreground"
                      }`}
                    >
                      <Filter className="h-4 w-4" />
                    </button>
              ...
              <th className="px-3 py-3 text-left text-xs font-semibold text-foreground uppercase border-r border-border w-28">
                <div className="flex items-center gap-2 justify-between">
                  RATING
                  <button
                    aria-label="label for rating filter"
                    ref={(el) => {
                      filterButtonRefs.current.rating = el;
                    }}
                    onClick={() =>
                      setOpenPopover(openPopover === "rating" ? null : "rating")
                    }
                    className={`p-1 rounded hover:bg-background/50 ${
                      filterRating ? "text-primary" : "text-muted-foreground"
                    }`}
                  >
                    <Filter className="h-4 w-4" />
                  </button>
              ...
              <th className="px-3 py-3 text-left text-xs font-semibold text-foreground uppercase border-r border-border">
                REVIEW
              </th>
              ...
              <th className="px-3 py-3 text-left text-xs font-semibold text-foreground uppercase w-32">
                <div className="flex items-center gap-2 justify-between">
                  DATE
                  <button
                    aria-label="label for date filter"
                    ref={(el) => {
                      filterButtonRefs.current.date = el;
                    }}
                    onClick={() =>
                      setOpenPopover(openPopover === "date" ? null : "date")
                    }
                    className={`p-1 rounded hover:bg-background/50 ${
                      filterDate ? "text-primary" : "text-muted-foreground"
                    }`}
                  >
          ...
          <tbody className="divide-y divide-border">
            {filteredReviews.length > 0 ? (
              filteredReviews.map((r) => (
                ...
                >
                  <td className="px-3 py-3 text-sm text-foreground border-r border-border font-mono">
                    {r.id}
                  </td>
                  {showTeacherCol && (
                    <td className="px-3 py-3 text-sm text-foreground border-r border-border">
                      {r.teacher_name}
                    </td>
                  )}
                  {showStudentCol && (
                  ...
                  <td className="px-3 py-3 text-sm text-foreground border-r border-border break-words min-w-[200px]">
                    {r.review_content}
                  </td>
              ...
              <tr>
                <td
                  colSpan={visibleCols}
                  className="py-12 text-center text-muted-foreground bg-background"
                >
